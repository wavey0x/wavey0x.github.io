...
As Prisma’s TVL dwindled thanks to the PSMs, a subtle accounting bug surfaced. I noticed a mismatch between the sum of user debt and what the trove thinks its total debt is. A search for the culprit was on. With some meticulous code inspection and some help from Tenderly, I was able to identify the issue within the `openTrove()` function.
...
The code incorrectly writes a state update to `totalActiveDebt` using a stale value that doesn't include the interest accrued since the prior checkpoint. In normal operations, this bug was transparent because `totalActiveDebt` was high enough that there was no underflow risk when subtracting a repayment amount. But if the descrepency is big enough, and as TVL drops, even a modest sized repayments is enough to trigger it. In fact, the bug would render repayments, redemptions, and even liquidations impossible because when the final borrower repays, say $1,000 the trove manager's attempt to subtract that amount from `totalActiveDebt` will underflow. These reverting transactions effectively trap users' collateral.

Upon further inspection, the bug was found to be present only in older versions of trove managers. Come to find out later, a patch was introduced by Prisma in one of the later versions of their Trove Manager code.
